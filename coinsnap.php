...
class Coinsnap extends PaymentModule
{
	...
	public function returnsuccess(){
		...
		if ($status == 'New') $status_id = $this->status_new;
		else if ($status == 'Expired') $status_id = $this->status_expired;
		else if ($status == 'Processing') $status_id = $this->status_processing;
		else if ($status == 'Settled') $status_id = $this->status_settled;	
    ...
    public function getContent()
    {
		if (Tools::isSubmit('submit' . $this->name)) {	
			...
			$saveOpt = false;
			$err_msg = '';
			if (empty(Tools::getValue('coinsnap_store_id'))) $err_msg = 'Store ID must have value';			
			if (empty(Tools::getValue('coinsnap_api_key'))) $err_msg = 'API Key must have value';				
			if (empty(Tools::getValue('coinsnap_status_new'))) $err_msg = 'Order Status New must have value';
			if (empty(Tools::getValue('coinsnap_status_expired'))) $err_msg = 'Order Status Expired must have value';
			if (empty(Tools::getValue('coinsnap_status_settled'))) $err_msg = 'Order Status Settled must have value';
			if (empty(Tools::getValue('coinsnap_status_processing'))) $err_msg = 'Order Status Processing must have value';
    ...
    public function coinsnapExternalPaymentOption()
    {
		...
		$errmsg = null;
		if (isset($_GET['coinsnaperror'])) $errmsg = $_GET['coinsnaperror'];
        ...
        $newOption->setCallToActionText($this->l('Pay with Bitcoin + Lightning'))			
            ->setForm($this->context->smarty->fetch('module:coinsnap/tpl/payment_infos.tpl'));
	...
	function add_log($logtype,$message){
